...
PClient createANewCellFromTheUser(char* line) {//Create a new cell from the user
	...
	char* parameter, * value;//Declaration of variables that will hold parenter and test value
	...
	do {
		...
		if (value) {
			if (!checkingIfTheNameIsProper(parameter)) {
				if (!strcmp(parameter, "first name")) { //testing to which object in the cell we will add
					if (!checkingIfTheNameIsProper(value))//Checking the value of the first name
					...
					if (!checkingIfTheNameIsProper(value))//Checking the value of the last name
					...
					if (!checkingTheAmountOfNumbersIsProper_CHAR(value, ID)) {//Checking the value of the ID
					...
					if (!checkingTheAmountOfNumbersIsProper_CHAR(value, PHONE)) {//Checking the value of the phone
					...
					if (*strDebt == '-')
						strDebt++;
					if (!checkingTheAmountOfNumbersIsProper_CHAR(strDebt, (int)strlen(strDebt)))//Checking the value of the debt
					...
					char* strDate = value;//Creating a pointer to the head of the string
...
void sortingByRequest(ListManager list) {//Sort by request from the user
	...
	if (checkingIfTheNameIsProper(parameter)) //Entered an incorrect title
		...
		if (value) {
			...
			if (!(strcmp(parameter, "first name"))) //Checking whether the sorting request is by first name
				if (checkingIfTheNameIsProper(value)) //checking the correctness of the name and changing it from lowercase to uppercase
				...
				if (checkingTheAmountOfNumbersIsProper_CHAR(value, ID))
				...
				if (checkingTheAmountOfNumbersIsProper_CHAR(value, PHONE))
				...
				if (checkingTheAmountOfNumbersIsProper_CHAR(tempCh, (int)strlen(tempCh))) //Check that the value we received contains only numbers
	...
	if (cellNew) {
		...
		if (cellNew) {
			if (cellNew->error.ERROR) {
				...
				printf("\tThe new line has been successfully received.\n");
